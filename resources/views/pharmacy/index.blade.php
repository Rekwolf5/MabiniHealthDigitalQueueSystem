...
<div class="dashboard-grid">
    ...
    <div class="dashboard-section" style="padding: 0; overflow: hidden;">
        <div style="display: flex; border-bottom: 2px solid #e5e7eb;">
            ...
            <a href="{{ route('pharmacy.index', ['tab' => 'inventory']) }}" 
               style="flex: 1; padding: 1rem; text-align: center; text-decoration: none; font-weight: 600;
                      color: {{ ($tab ?? 'pending') === 'inventory' ? '#059669' : '#6b7280' }};
                      border-bottom: {{ ($tab ?? 'pending') === 'inventory' ? '3px solid #059669' : '3px solid transparent' }};
                      background: {{ ($tab ?? 'pending') === 'inventory' ? '#f0fdf4' : 'white' }};">
    ...
    @if(($tab ?? 'pending') === 'inventory')
    <!-- MEDICINE INVENTORY TAB -->
    <div class="dashboard-section">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h3 style="font-size: 1.5rem; font-weight: 700; color: #059669;">
                <i class="fas fa-boxes"></i> Medicine Inventory Management
            </h3>
        ...
        <!-- Search and Filter -->
        <div style="background: #f9fafb; padding: 1.5rem; border-radius: 8px; border: 1px solid #e5e7eb; margin-bottom: 1.5rem;">
            <form method="GET" action="{{ route('pharmacy.index') }}" style="display: flex; gap: 1rem; flex-wrap: wrap;">
                ...
                <div style="flex: 0 0 200px;">
                    <select name="stock_filter" 
                        ...
                        <option value="available" {{ request('stock_filter') == 'available' ? 'selected' : '' }}>In Stock</option>
                    </select>
                </div>
                ...
                @if(request('search') || request('stock_filter'))
                    <a href="{{ route('pharmacy.index', ['tab' => 'inventory']) }}" class="btn btn-secondary">
        ...
        <div class="data-table-container">
            <table class="data-table">
                <thead>
                    <tr style="background: #059669;">
                        <th style="color: white;"><i class="fas fa-pills"></i> Medicine Name</th>
                        <th style="color: white;"><i class="fas fa-tag"></i> Generic Name</th>
                        <th style="color: white;"><i class="fas fa-tablets"></i> Dosage</th>
                        <th style="color: white;"><i class="fas fa-boxes"></i> Stock</th>
                        <th style="color: white;"><i class="fas fa-ruler"></i> Unit</th>
                        <th style="color: white;"><i class="fas fa-calendar-alt"></i> Expiry Date</th>
                        <th style="color: white;"><i class="fas fa-chart-line"></i> Status</th>
                        <th style="color: white; text-align: center;"><i class="fas fa-cog"></i> Actions</th>
                ...
                <tbody>
                    @php
                        ...
                        if(request('search')) {
                            ...
                            $medicines->where(function($q) use ($search) {
                        ...
                        if(request('stock_filter') == 'out_of_stock') {
                            $medicines->where('stock', 0);
                        } elseif(request('stock_filter') == 'low') {
                            $medicines->where('stock', '>', 0)->where('stock', '<=', 10);
                        } elseif(request('stock_filter') == 'available') {
                            $medicines->where('stock', '>', 10);
                        }
                    ...
                    @forelse($medicines as $medicine)
                        <tr>
                            ...
                            <td>
                                <span style="font-size: 1.25rem; font-weight: 700; color: {{ $medicine->stock == 0 ? '#dc2626' : ($medicine->stock <= 10 ? '#f59e0b' : '#10b981') }};">
                                    {{ $medicine->stock }}
                                </span>
                            </td>
                            <td>{{ $medicine->unit }}</td>
                            <td>
                                @if($medicine->expiry_date)
                                    @php
                                        $daysLeft = now()->diffInDays($medicine->expiry_date, false);
                                    @endphp
                                    <span style="color: {{ $daysLeft < 0 ? '#dc2626' : ($daysLeft <= 30 ? '#f59e0b' : '#059669') }};">
                                        {{ $medicine->expiry_date->format('M d, Y') }}
                                        @if($daysLeft < 0)
                                            <br><small style="color: #dc2626; font-weight: 600;">EXPIRED</small>
                                        @elseif($daysLeft <= 30)
                                            <br><small style="color: #f59e0b;">{{ $daysLeft }} days left</small>
                                        @endif
                                    </span>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="8" style="text-align: center; padding: 3rem; color: #6b7280;">
                                ...
                                <p style="font-size: 1.125rem; font-weight: 600;">No medicines found</p>
