...
@section('title', $service->name . ' Queue - Mabini Health Center')
@section('page-title', $service->name . ' Service')
...
<div class="dashboard-grid">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                <h1 style="font-size: 2rem; font-weight: 700; margin-bottom: 0.5rem;">
                    <i class="fas fa-hospital"></i> {{ $service->name }}
                </h1>
            ...
            <div style="text-align: right;">
                <div style="background: rgba(255,255,255,0.2); padding: 0.75rem 1rem; border-radius: 6px; margin-bottom: 0.5rem;">
                    <strong>Capacity:</strong> {{ $stats['current_capacity'] }}/{{ $stats['max_capacity'] }} per hour
                </div>
                <div style="background: {{ $stats['availability'] ? 'rgba(16, 185, 129, 0.3)' : 'rgba(239, 68, 68, 0.3)' }}; padding: 0.5rem 1rem; border-radius: 4px;">
                    <i class="fas fa-{{ $stats['availability'] ? 'check-circle' : 'exclamation-triangle' }}"></i>
                    {{ $stats['availability'] ? 'Available' : 'At Capacity' }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div style="margin-bottom: 1.5rem;">
        <form method="POST" action="{{ route('services.call-next', $service->id) }}" style="display: inline;">
            @csrf
    ...
    <div class="dashboard-section">
        ...
        <div class="quick-actions" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
            <form action="{{ route('services.call-next', $service->id) }}" method="POST" style="margin: 0;">
                @csrf
            ...
            <a href="{{ route('front-desk.index') }}" class="action-btn" style="background: #3b82f6; color: white;" title="View main front desk queue to see unassigned patients">
            ...
            <a href="{{ route('services.status', $service->id) }}" class="action-btn" style="background: #6366f1; color: white;" title="View detailed service statistics and performance">
                ...
                <tr>
                    ...
                    <td>
                        <div style="font-size: 0.875rem;">
                            <div><strong>Arrived:</strong> {{ $queue->arrived_at->format('h:i A') }}</div>
                            @if($queue->called_at)
                                <div><strong>Called:</strong> {{ $queue->called_at->format('h:i A') }}</div>
                            @endif
                            @if($queue->completed_at)
                                <div><strong>Done:</strong> {{ $queue->completed_at->format('h:i A') }}</div>
                            @endif
                        </div>
                    </td>
                    <td style="max-width: 200px;">
                        <small>{{ $queue->notes ? Str::limit($queue->notes, 50) : 'No notes' }}</small>
                    </td>
                    <td style="position: sticky; right: 0; background: white; box-shadow: -2px 0 4px rgba(0,0,0,0.05); z-index: 5;">
                        <div class="action-buttons" style="justify-content: center; display: flex; flex-wrap: nowrap; gap: 0.25rem; padding: 0.25rem;">
                            @if($queue->status === 'called')
                                <button onclick="announceQueue('{{ $queue->queue_number }}', '{{ $queue->patient_name }}')" 
                                ...
                                <form method="POST" action="{{ route('services.skip', $queue->id) }}" style="display: inline; margin: 0;">
                                    @csrf
                            ...
                            @if($queue->status === 'waiting' || $queue->status === 'called')
                                ...
                                @if($queue->workflow_stage === 'registration' && in_array(strtolower($service->name), ['general practitioner', 'general practice', 'ob-gyn', 'maternal & child health']))
                                    <form method="POST" action="{{ route('services.vitals.call', $queue->id) }}" style="display: inline; margin: 0;">
                                        @csrf
                                ...
                                @if($queue->workflow_stage === 'vitals' && in_array(strtolower($service->name), ['general practitioner', 'general practice', 'ob-gyn', 'maternal & child health']))
                                    <form method="POST" action="{{ route('services.vitals.complete', $queue->id) }}" style="display: inline; margin: 0;">
                                        @csrf
                                ...
                                @if($queue->workflow_stage === 'consultation' && $queue->status === 'called')
                                    <form method="POST" action="{{ route('services.start', $queue->id) }}" style="display: inline; margin: 0;">
                                        @csrf
                            ...
                            @if($queue->status === 'in_progress')
                                <form method="POST" action="{{ route('services.complete', $queue->id) }}" style="display: inline; margin: 0;">
                                    @csrf
                            ...
                            @if(in_array($queue->status, ['waiting', 'called', 'in_progress']))
                                <button onclick="showTransferModal({{ $queue->id }}, '{{ $queue->patient_name }}')" 
                            ...
                            @if(in_array($queue->status, ['waiting', 'called']))
                                <form method="POST" action="{{ route('services.no-show', $queue->id) }}" style="display: inline; margin: 0;">
                                    @csrf
                                    <button type="submit" class="btn btn-sm" style="background: #6b7280; color: white; min-width: 35px; padding: 0.375rem;" 
                                            ...
                                            onclick="return confirm('Mark {{ $queue->patient_name }} as NO SHOW?\n\nThis will:\n• Remove patient from queue\n• Archive as no-show\n• Cannot be undone')">
                                        <i class="fas fa-user-times"></i>
                                    </button>
                                </form>
                            @endif
                            
                            <button onclick="showNotesModal({{ $queue->id }}, '{{ addslashes($queue->notes) }}')" 
                ...
                <tr>
                    <td colspan="7" style="text-align: center; padding: 3rem;">
                        <div style="display: flex; flex-direction: column; align-items: center; gap: 1rem;">
                            ...
                            <div>
                                <h3 style="font-size: 1.25rem; font-weight: 700; color: #6b7280; margin-bottom: 0.5rem;">No Patients in {{ $service->name }} Queue</h3>
                                <p style="color: #9ca3af;">Patients will appear here when assigned from the front desk</p>
                                <a href="{{ route('front-desk.index') }}" class="btn btn-primary" style="margin-top: 1rem;">
...
function loadAvailableServices() {
    fetch(`/services/available/{{ $service->id }}`)
...
<div id="vitalsModal" class="modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5);">
    <div class="modal-content" style="background-color: #fefefe; margin: 2% auto; padding: 20px; border: none; border-radius: 8px; width: 90%; max-width: 600px;">
        ...
        <form method="POST" id="vitalsForm" action="{{ route('services.vitals.store') }}">
            @csrf
